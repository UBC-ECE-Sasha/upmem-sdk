...
static void
print(struct verbose_control *vcs, const char *log_type, const char *__restrict __format, va_list __arg)
{
    ...
    fprintf(out, "%s:%s:%s: %s\n", vcs->module_name, log_type, time_string, content);
    fflush(out);
    free(content);
}

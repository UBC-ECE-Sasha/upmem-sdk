				  ...
				  u32 *results)
{
	...
	struct dpu_bit_config *bit_config =
		&(GET_DESC(rank)->configuration.pcb_transformation);
...
__API_SYMBOL__ u32 ufi_select_all(struct dpu_rank_t *rank, u8 *ci_mask)
{
	...
	/* Initialize structs with default value (a special NOP) */
				      ...
				      u8 address, u8 data)
{
	...
	FF(UFI_exec_void_frame(rank, ci_mask, CI_DMA_CTRL_WRITE_STRUCT,
			       CI_DMA_CTRL_WRITE_FRAME(b0, b1, b2, b3, b4,
						       b5)));
...
{
	u32 status;
	u8 mux_value = dpu_has_mux ? 1 : 0;

	FF(ufi_write_dma_ctrl(rank, ci_mask, 0x80, mux_value));
	FF(ufi_write_dma_ctrl(rank, ci_mask, 0x81, 0));
	FF(ufi_write_dma_ctrl(rank, ci_mask, 0x82, mux_value));
	FF(ufi_write_dma_ctrl(rank, ci_mask, 0x84, mux_value));
				     ...
				     u16 *pcs)
{
	u32 status;
	u8 nr_cis = GET_DESC(rank)->topology.nr_of_control_interfaces;
	u32 iram_size = GET_DESC(rank)->memories.iram_size;
				     ...
				     u64 *structures)
{
	...
	bool do_write_structure = false;
	for_each_ci (each_ci, nr_cis, ci_mask) {
		if (ci->slice_info[each_ci].structure_value !=
		    structures[each_ci]) {

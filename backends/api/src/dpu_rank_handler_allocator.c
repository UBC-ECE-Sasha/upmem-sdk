...
/**
 * Ensures that there is no concurrent allocation/release of DPU handlers.
 */
static pthread_mutex_t static_handler_mutex = PTHREAD_MUTEX_INITIALIZER;

static inline void
exclusively()
{
    pthread_mutex_lock(&static_handler_mutex);
}

static inline void
exclusively_end()
{
    pthread_mutex_unlock(&static_handler_mutex);
}
...
dpu_rank_handler_get_rank(struct dpu_rank_t *rank, dpu_rank_handler_context_t handler_context, dpu_properties_t properties)
{
    ...
    if (!dpu_get_next_rank_id(&rank->rank_id)) {
        return false;
    }
    ...
    if (!fetch_boolean_property(properties,
            DPU_PROFILE_PROPERTY_DISABLE_API_SAFE_CHECKS,
            &description->configuration.disable_api_safe_checks,
            false)) {
        dpu_free_description(description);
        dpu_release_rank_id(rank->rank_id);
        return false;
    ...
    for (dpu_slice_id_t each_slice = 0; each_slice < nr_cis; ++each_slice) {
        ...
        rank->runtime.control_interface.slice_info[each_slice].host_mux_mram_state = 0;
    }
...
set_generic_description_for_enabled_dpus(struct dpu_rank_t *rank, dpu_description_t description, dpu_properties_t properties)
{
    ...
    for (uint8_t each_ci = 0; each_ci < description->topology.nr_of_control_interfaces; ++each_ci) {
        ...
        rank->runtime.control_interface.slice_info[each_ci].enabled_dpus
            = dpu_mask_difference(dpu_mask_all(description->topology.nr_of_dpus_per_control_interface), disabled_mask_for_ci);
    }

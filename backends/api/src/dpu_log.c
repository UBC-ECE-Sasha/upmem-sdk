...
static void
parse_and_print_argument_section(dpulog_reader_t *reader, FILE *output)
{
    ...
    for (fmt_index = 0; (fmt_index < reader->format_count); fmt_index++) {
        ...
        switch (l_fmt) {
            case 's':
                (void)fprintf(output, reader->format[fmt_index], reader->buffer);
            ...
            case 'b': {
                ...
                for (int each_bit = 0; each_bit < 32; ++each_bit) {
                    (void)fprintf(output, "%u", (i >> each_bit) & 1);
                }
            ...
            case 'u': {
                ...
                if (is_64_bit) {
                    ...
                    (void)fprintf(output, reader->format[fmt_index], l);
                    reader->buffer += sizeof(long);
                } else {
                    int i = *(int *)(reader->buffer);
                    (void)fprintf(output, reader->format[fmt_index], i);
                    reader->buffer += sizeof(int);
                }
            } break;
            case 'e':
            ...
            case 'f': {
                ...
                (void)fprintf(output, reader->format[fmt_index], d);
                reader->buffer += sizeof(double);
            } break;
            default: {
                // do not display anything for unsupported formats (A, p, e, etc)
                (void)fprintf(output, "%s", reader->format[fmt_index]);
                reader->buffer += sizeof(int);
            }
        }
    }
    ...
    while (true) {
        switch (parse_format_section(&reader)) {
            case PARSE_FORMAT_SECTION_SUCCESS:
                parse_and_print_argument_section(&reader, output);
                break;
...
dpulog_read_for_dpu(struct dpu_t *dpu, FILE *stream)
{
    ...
    api_status = dpu_copy_from_mram(dpu, &buffer[buffer_index], printf_buffer_address, buffer_size, DPU_PRIMARY_MRAM);
    if (api_status != DPU_OK) {
        free(buffer);
    ...
    api_status = dpulog_read_and_display_contents_of(buffer, buffer_size, stream);
    if (api_status != DPU_OK) {
        free(buffer);
